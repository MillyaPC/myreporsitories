...
void CMyPlayerDlg::InitButtons()
{
	...
	m_btnNext.Init(IDB_NEXT,IDB_NEXTOVER,"下一首");

	rect.left-=55;
	//m_btnPlay.MoveWindow(&rect,TRUE);
	m_btnPlay.Init(IDB_START,IDB_STARTOVER,"");

	rect.left-=55;
	//m_btnPrev.MoveWindow(&rect,TRUE);
	m_btnPrev.Init(IDB_PREV,IDB_PREVOVER,"上一首");
...
//	m_btnStop.Init(IDB_STOP_NORMAL,IDB_STOP_OVER,"停止");
	...
	m_btnHide.Init(IDB_HIDE,IDB_HIDEOVER,"隐藏窗体");

	rect.left+=14;
	m_btnMin.MoveWindow(&rect,TRUE);
	m_btnMin.Init(IDB_MIN,IDB_MINOVER,"最小化");

	rect.left+=14;
	m_btnExit.MoveWindow(&rect,TRUE);
	m_btnExit.Init(IDB_CLOSE,IDB_CLOSEOVER,"关闭");

	rect.top=100;
	rect.left=269;
	m_btnPLShow.MoveWindow(&rect,TRUE);
	m_btnPLShow.Init(IDB_LISTHIDE,IDB_LISTHIDEOVER,"关闭播放列表");

	rect.top=81;
	rect.left=145;
	//m_slide.MoveWindow(&rect,TRUE);
	m_slide.Init(IDB_SLIDEBK,IDB_SLIDER1,"当前进度");
...
void CMyPlayerDlg::DrawMainPicture(CDC*pDC)
{
	...
	bitmap.LoadBitmap(IDB_PLAYERMAIN);
...
void CMyPlayerDlg::OnStaticPlay() 
{
	...
	fPlay=!fPlay;
	if(fPlay){
		m_stcPlay.SetToolTipText("播放");
		MCIWndResume(hAudio);
	}
	else{
		m_stcPlay.SetToolTipText("暂停");
		MCIWndPause(hAudio);
	}
}
	...
	// TODO: Add your control notification handler code here
	 if(m_pPlayListDlg!=NULL)
		if(m_pPlayListDlg->IsWindowVisible()){
		    ...
		    m_btnPLShow.SetBitmap(IDB_SHOW,IDB_SHOWOVER);
          	m_btnPLShow.SetToolTipText("显示播放列表");
		}else{
			m_pPlayListDlg->ShowWindow(SW_SHOW);
			m_btnPLShow.SetBitmap(IDB_LISTHIDE,IDB_LISTHIDEOVER);
...
//		   Play(0);
	   ...
	   else	if(m_strPlayListArray.GetSize()>0)
	   {
         	Play(m_nPlayOrder[0]);
			m_nNowPlaying=0;		
		}
...
void CMyPlayerDlg::CirculatePlay()
{
	if(hAudio==NULL)
		return;
//	if(m_nPlayMode==MODE_RAND&&MCIWndGetPosition(hAudio)==MCIWndGetLength(hAudio)-50)
//        theApp.RandOrder();
...
void CMyPlayerDlg::OnBtnShowmenu() 
{
   ...
   m_ModeMenu.AppendMenu(MF_STRING,IDC_MENU_RAND,"自动");
   m_ModeMenu.AppendMenu(MF_STRING,IDC_MENU_SEQ,"顺序");
   m_ModeMenu.AppendMenu(MF_STRING,IDC_MENU_SINGLE,"读书");
