...
{
	program_sense sensor;
	//m_strPlayListArray.RemoveAll();
 	sensor.sense_program();
	//CPlayListDlg* m_pPlayListDlg2;
	string key = sensor.send_keyword();
	string state = sensor.get_s(key);
	CString strpath1=sensor.dispatch(state).c_str();
	//CString shortname=GetFileTitle(strpath1);
	m_strPlayListArray.Add(strpath1);
...
void CMyPlayerApp::readOrder()
{
   program_sense sensor;
   CString strpath1=sensor.dispatch("Äþ¾²").c_str();
   m_strPlayListArray.Add(strpath1);
   theApp.SequenceOrder();
}
