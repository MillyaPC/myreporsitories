...
void CPlayListDlg::DrawBackground(CDC*pDC)
{
	...
	bitmap.LoadBitmap(IDB_LISTMAIN);
...
void CPlayListDlg::InitButtons()
{
	...
	m_btnClose.Init(IDB_CLOSE,IDB_CLOSEOVER,"关闭播放列表");
    
	rect.top=224;
	rect.left=24;
	//m_btnAdd.MoveWindow(&rect,TRUE);
	m_btnAdd.Init(IDB_ADD,IDB_ADDOVER,"添加歌曲");

	rect.left=57;
	//m_btnDel.MoveWindow(&rect,TRUE);
	m_btnDel.Init(IDB_HIDE,IDB_HIDEOVER,"删除歌曲");

	rect.left=90;
	//m_btnList.MoveWindow(&rect,TRUE);
	m_btnList.Init(IDB_MIN,IDB_MINOVER,"列表操作");
...
void CPlayListDlg::OnBtnClose() 
{
	...
	pButton->SetBitmap(IDB_SHOW,IDB_SHOWOVER);
...
LRESULT CPlayListDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if(message==WM_CTLCOLORLISTBOX){
		HBRUSH hListBrush;
		hListBrush=CreateSolidBrush(RGB(255,255,255));
		::SetBkMode((HDC)wParam,TRANSPARENT);	
		SetTextColor((HDC)wParam,RGB(0,0,0));
		SetBkColor((HDC)wParam,RGB(0,0,0));
	//	SetFont((HDC)wParam);
		return  (LRESULT)::GetStockObject(NULL_BRUSH);/*(LRESULT)hListBrush;//终于透明啦！！！*/
	}
...
/*	m_playlist.ResetContent();
	for(int j=0;j<m_strPlayListArray.GetSize();j++){
		strFileTitle=GetFileTitle(m_strPlayListArray.GetAt(i));
        if(j+1<10)
           strFormat.Format("0%d  %s",j+1,strFileTitle);
		else
			strFormat.Format("%d  %s",j+1,strFileTitle);
		m_playlist.AddString(strFormat);
	}*/
